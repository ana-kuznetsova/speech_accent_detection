...
def load_data(path):
    return librosa.load(path, sr=16000, dtype="float32")
...
def main():
    ...
    train_speech = [load_data(os.path.join(global_path, p)) for p in train_filenames]
    ...
    test_speech = [load_data(os.path.join(global_path, p)) for p in test_filenames]
